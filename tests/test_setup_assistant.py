...
class SetupAssistantTests(unittest.TestCase):
    ...
    def test_beginner_labels_and_compatibility_launcher(self) -> None:
        ...
        self.assertIn('68k captured & halted', source)
        self.assertIn('"mdv_image_path": self.mdv_image_path', source)
        self.assertIn('"Upload MDV without mounting", self.upload_microdrive', source)
        ...
        self.assertEqual(link_source.count('text="NanoQL Link port"'), 1)
        refresh_at = link_source.index('text="Refresh"')
        check_at = link_source.index('"Check connection", self.link_status')
        self.assertGreater(check_at, refresh_at)
        ...
        flash_start = source.index("    def flash_bl616(self)")
