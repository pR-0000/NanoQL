...
In autonomous mode, `nanoql_drive.c` converts a selected folder below `/NanoQL/Microdrives` into an exact 174,930-byte QLAY image. It accepts up to 126 files, eight nested directory levels, flattened printable-ASCII names up to 36 characters, and at most 253 allocated 512-byte sectors. It writes a temporary image, preserves the previous cartridge until replacement succeeds, mounts `/NanoQL/Drive1/MDV1.mdv` read/write for the current session, and resets only the QL after the menu command completes. QDOS writes update that image rather than the source folder. The generated image is excluded from normal INI persistence. The source patch and generated board images are distributed under the upstream Apache License 2.0.
