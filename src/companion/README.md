...
The generated `nanoql_xml.hex` contains the gzip-compressed `nanoql.xml` configuration served to FPGA Companion. Regenerate it from the repository root with `tools/build_companion_config.ps1`. The menu mounts target 0 as the QL ROM. The microSD preparation tools also create `nanoql.ini`; the XML loads it at startup so Companion mounts `/sd/QL.rom` as drive 0. The XML `default` attribute alone only configures the selector and does not mount a file. `ql_sd_rom_loader.sv` accepts 48 KiB and 64 KiB images, pads a 48 KiB image to 64 KiB with `0xff`, stores it in dedicated runtime-writable BSRAM, and verifies every word before releasing the 68000.
