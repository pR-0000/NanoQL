...
The first overlay entry explicitly displays `Execution: Running` or `Paused`. Physical/remote F9 changes the same session-only state, which always returns to Running after power-up. Native QL timing, video, HDMI scanout, audio, OSD, and Companion storage stay active while the CPU is paused. The screenshot action asks the BL616 to save an aspect-corrected 1024 x 698 PNG under `/NanoQL/Screenshots`; it uses the dedicated coherent screenshot buffer, does not include the overlay, and reports `[OK]` after a successful save.
